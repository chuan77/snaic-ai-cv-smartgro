...
```bash
uv run pytest -v          # 230 tests across tests/{data,models,deploy,pipeline}, 1 deselected @pytest.mark.slow
...
- `src/pipeline/` — `data_augmentor.py` (`AdvancedDataAugmentor`, unimplemented stub), `training_pipeline.py`'s `ModelTrainingPipeline` (implemented — orchestrates rebuild-catalog → resynthesize → retrain → audit for `retrain_from_label_studio.py`; always writes to a fresh `runs/detect/retrain_<timestamp>/` and `synthetic_dataset_retrain/`, never to `runs/detect/train/` or the checkout API's served weights — promotion is manual), `candidate_promotion.py` (`read_promoted_state`/`write_promoted_state`/`write_candidate_report`/`should_promote`/`promote`/`seed_baseline_state` — pipeline-state persistence and the mechanical, non-human promotion decision for the autonomous loop), `al_scheduler.py` (`run_scheduler_tick`/`run_retrain_cycle`/`restart_api_service` — orchestrates the autonomous loop end-to-end, behind root `al_scheduler_check.py`; see "Autonomous active-learning pipeline" below).
...
Automates the manual capture → push → human-review → retrain → manually-promote loop above into capture → VLM-auto-label → retrain → dual-gate → mechanically-promote, with no human review step. See README's "🤖 Autonomous Active-Learning Pipeline" section for the operator-facing walkthrough and diagram. The existing Label Studio integration (above) is untouched and still works standalone — this is an additive, opt-in path, not a replacement; it's only active once the `com.smartcart.al-scheduler` launchd service is installed. Implementation notes relevant to changing this code:

- **LM Studio's chat endpoint is not the standard OpenAI-compatible shape.** `src/models/vlm_verifier.py`'s `ask_vlm()` posts to `POST {SMARTCART_VLM_BASE_URL}/api/v1/chat` with `{"model": ..., "input": [{"type": "text", "content": ...}, {"type": "image", "data_url": "data:image/jpeg;base64,..."}]}`, reading the answer from `response.json()["output"][0]["content"]`. This server also exposes a standard `/v1/chat/completions` (confirmed working during development), but `/api/v1/chat` is the integration target — don't "fix" it to the OpenAI shape without checking against the actual running LM Studio instance first.
- **Every VLM/box-parsing failure mode returns `None`, never raises.** `ask_vlm()`, `match_category()`, `parse_grounding_box()` (`vlm_verifier.py`) and `auto_import_mid_band_detection()`/`auto_import_low_signal_frame()` (`auto_labeler.py`) all treat "VLM unreachable," "VLM and YOLO disagree," and "no confident category match" identically: discard that capture, don't guess. `auto_import_capture()` wraps its whole per-capture body in `try/except Exception: ... finally: mark_consumed(...)` so one corrupt staged image (bad JSON, unreadable JPEG) can never crash the batch or leave a capture stuck in permanent retry.
- **`ModelTrainingPipeline` (`training_pipeline.py`) is deliberately unmodified.** Candidate gallery/catalog isolation from the live `./artifacts/` path — the bug this whole pipeline was partly built to fix, since `rebuild_catalog()` writes wherever `artifacts_dir` points at construction time — is achieved entirely in `al_scheduler.py`'s `run_retrain_cycle()`: it points a fresh `ModelTrainingPipeline` at a temp dir (`candidates_root/_tmp_candidate`), then renames the temp dir to `candidates_root/<run_name>` after `.run()` returns (pass or fail). Only `candidate_promotion.promote()` ever copies files into live `./artifacts/`.
- **Promotion is a triple gate, not a simple "did it improve" check:** `should_promote()` (non-regression vs. the currently-promoted run's own recorded metrics) AND `train_result["passed"]` (YOLO mAP50 absolute floor, existing `SMARTCART_RETRAIN_MIN_MAP50`) AND `variant_acc >= get_retrain_min_variant_acc()` (DINOv2 held-out accuracy absolute floor, `SMARTCART_RETRAIN_MIN_VARIANT_ACC`). All three live in `al_scheduler.run_scheduler_tick()`.
- **Run `seed_al_baseline.py` once before installing the `al-scheduler` service.** `al_pipeline_state.json` defaults to `map50: 0.0, variant_accuracy: 0.0` when it doesn't exist yet — comparing a candidate against that phantom baseline instead of the live model's real, already-good metrics would let a mediocre candidate "win" and regress a strong incumbent on the very first autonomous promotion. `seed_al_baseline.py` measures the currently-served weights/gallery for real and writes those numbers into the state file first. Documented as a required install step in `launchd/README.md`.
- **`launchctl kickstart` needs a domain-qualified target, not a bare label.** `restart_api_service()` (`al_scheduler.py`) runs `launchctl kickstart -k gui/<uid>/com.smartcart.api` (via `os.getuid()`), not `-k com.smartcart.api` — the bare label silently fails ("Could not find service") and leaves the API serving stale weights after an otherwise-successful promotion.
- **`vlm_verifier.py` also does `import httpx`**, same as the Label Studio modules — the shared-module-identity monkeypatch gotcha above applies here too if a future test ever needs to patch both an LM Studio call and a Label Studio call in the same test.
